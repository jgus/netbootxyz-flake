...
# Per-version branch orchestrator for netbootxyz-flake. See bencoding-flake/update-branches.sh for the general design.
#
# netbootxyz-specific bits:
# - list_upstream_versions queries GitHub releases (netbootxyz/netboot.xyz). Tags are bare semver (e.g. 3.0.2, no v prefix).
# - placeholder pin.nix has 2 fields (version, assets={}).
# - flake.nix is NOT branch-owned (no cascade); only pin.nix and flake.lock are.
#
# Structural: each existing exact branch is `git merge`d with origin/main before its update-version runs, so orchestrator/workflow improvements that land on main propagate forward through every branch's tree.
#
# Failures: per-branch update-version failures are surfaced as GH Actions ::warning:: annotations + a step summary, and cause a non-zero exit at the end of the run.
...
list_upstream_versions() {
  gh api --paginate "/repos/netbootxyz/netboot.xyz/releases" --jq '.[].tag_name'
}
...
for v in "${tracked[@]}"; do
  ...
  if (( uv_exit != 0 )); then
    failed+=("${v}")
    echo "::warning title=Branch ${branch} skipped::update-version failed for ${v} (exit ${uv_exit}). Likely a missing checksum file at that release; see the orchestrator log above."
